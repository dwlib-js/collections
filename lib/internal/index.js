...
const ToNumber = require('#type/ToNumber');
...
const $ListIteratorPredicate = Symbol('[[ListIteratorPredicate]]');
...
const RequireComparerCallable = argument => {
  if (!IsCallable(argument)) {
    throw new TypeError('comparer is not callable');
  }
}
...
const ImmutableListFindAll = (immutableList, predicate) => {
  const array = RequireInternalSlot(immutableList, $ImmutableListArray);
  RequirePredicateCallable(predicate);
  const arrayIterator = ArrayEntries(array);
  const listIterator = ObjectCreate(ListIteratorPrototype);
  SetInternalSlot(listIterator, $ListArrayIterator, arrayIterator);
  SetInternalSlot(listIterator, $ListIteratorPredicate, predicate);
  return listIterator;
}
...
const ImmutableListSort = (immutableList, comparer, start, end) => {
  const array = RequireInternalSlot(immutableList, $ImmutableListArray);
  if (comparer !== undefined) {
    RequireComparerCallable(comparer);
  }
  start = ToIntegerOrInfinity(start);
  const size = array.length;
  if (start === -Infinity) {
  ...
  if (end - start < 10) {
    ...
    if (comparer) {
      while (++start < end) {
        ...
        let index = start - 1;
        while (index >= 0) {
          const prevValue = destination[index];
          let comparison = FunctionCall(comparer, null, prevValue, value);
          comparison = ToNumber(comparison);
          if (!comparison || comparison < 0) {
            break;
          }
          destination[index + 1] = prevValue;
          index--;
        }
  ...
  if (comparer) {
    while (index >= 0) {
      ...
      if (start < end) {
        const pivot = destination[start];
        while (start < end) {
          while (start < end) {
            let comparison = FunctionCall(comparer, null, destination[end], pivot);
            comparison = ToNumber(comparison);
            if (!comparison || comparison < 0) {
              break;
            }
          ...
          while (start < end) {
            let comparison = FunctionCall(comparer, null, pivot, destination[start]);
            comparison = ToNumber(comparison);
            if (!comparison || comparison < 0) {
              break;
            }
        ...
        destination[start] = pivot;
        left[index + 1] = start + 1;
        right[index + 1] = right[index];
        right[index++] = start;
        if (right[index] - left[index] > right[index - 1] - left[index - 1]) {
          let swap = left[index];
    ...
    while (index >= 0) {
      ...
      if (start < end) {
        const pivot = destination[start];
        if (pivot !== pivot || pivot === undefined) {
          continue;
        }
        while (start < end) {
          while (start < end && destination[end] >= pivot) {
            end--;
          }
          if (start < end) {
            destination[start++] = destination[end];
          }
          while (start < end && destination[start] <= pivot) {
            start++;
          }
          if (start < end) {
            destination[end--] = destination[start];
...
const ListFindAll = (list, predicate) => {
  const array = RequireInternalSlot(list, $ListArray);
  RequirePredicateCallable(predicate);
  const arrayIterator = ArrayEntries(array);
  const listIterator = ObjectCreate(ListIteratorPrototype);
...
const ListSort = (list, comparer, start, end) => {
  const array = RequireInternalSlot(list, $ListArray);
  if (comparer !== undefined) {
    RequireComparerCallable(comparer);
  }
    ...
    if (comparer) {
      while (index >= 0) {
        ...
        if (start < end) {
          ...
          while (start < end) {
            while (start < end) {
              let comparison = FunctionCall(comparer, null, array[end], pivot);
              comparison = ToNumber(comparison);
              if (!comparison || comparison < 0) {
                break;
              }
            ...
            while (start < end) {
              let comparison = FunctionCall(comparer, null, pivot, array[start]);
              comparison = ToNumber(comparison);
              if (!comparison || comparison < 0) {
                break;
              }
...
const ListIteratorNext = listIterator => {
  ...
  const predicate = GetInternalSlot(listIterator, $ListIteratorPredicate);
  if (predicate) {
    let iteratorResult = ArrayIteratorNext(arrayIterator);
    while (!iteratorResult.done) {
      const entry = iteratorResult.value;
      const index = entry[0];
      const value = entry[1];
      const truthy = FunctionCall(predicate, null, value, index);
      if (truthy) {
        return {
          value,
          done: false
      ...
      iteratorResult = ArrayIteratorNext(arrayIterator);
    }
...
const ReadOnlyListFindAll = (readOnlyList, predicate) => {
  const list = RequireInternalSlot(readOnlyList, $TargetList);
  RequirePredicateCallable(predicate);
  const array = GetInternalSlot(list, $ListArray);
  const arrayIterator = ArrayEntries(array);
...
class List {
  static from(source, callback) {
    ...
    let iterator = source[SymbolIterator];
    if (iterator != null) {
      iterator = FunctionCall(iterator, source);
      const destination = [];
      let index = 0;
      if (callback) {
  ...
  findAll(predicate) {
    return ListFindAll(this, predicate);
  }
  ...
  sort(compare, start, end) {
    ListSort(this, compare, start, end);
    return this;
  }
...
class ReadOnlyList {
  ...
  findAll(predicate) {
    return ReadOnlyListFindAll(this, predicate);
  }
...
class ImmutableList {
  ...
  findAll(predicate) {
    return ImmutableListFindAll(this, predicate);
  }
  ...
  sort(comparer, start, end) {
    return ImmutableListSort(this, comparer, start, end);
  }
